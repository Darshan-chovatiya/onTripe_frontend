...
function NotificationCard({ item }) {
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      ...
      {/* Recipients list — toggled */}
      {expanded && recipients.length > 0 && (
        <div className="border-t border-gray-100 px-5 pb-5 pt-4">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {recipients.map((r, i) => (
              <RecipientRow key={i} r={r} />
            ))}
          </div>
        </div>
      )}
...
export default function NotificationHistory() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else toastRef.current.error(data?.message || 'Could not load history')
    } catch {
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-10">
      <nav className="flex flex-wrap items-center gap-2 text-sm" aria-label="Breadcrumb">
        <Link to="/admin/notifications" className="font-medium text-primary-700 hover:text-primary-900">
          Notifications
        </Link>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">All broadcasts sent from this account</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-6 py-20 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Broadcasts you send will appear here.</p>
          <Link
