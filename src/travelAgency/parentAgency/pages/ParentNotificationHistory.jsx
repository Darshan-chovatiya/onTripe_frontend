...
import { getSentNotifications } from '@/travelAgency/parentAgency/services/parentAgencyApi.js'
...
export default function ParentNotificationHistory() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        // Only show notifications sent to child agents from this parent
        setHistory(
          all.filter((n) =>
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-10">
      <nav className="flex flex-wrap items-center gap-2 text-sm" aria-label="Breadcrumb">
        <Link to="/agency/manage-downstream" className="font-medium text-primary-700 hover:text-primary-900">
          Manage Children
        </Link>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">All broadcasts sent to your child agents</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-6 py-20 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Notifications you send to child agents will appear here.</p>
          <Link
            to="/agency/manage-downstream"
