...
export default function ChildLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      navigate('/agency/child/dashboard', { replace: true })
    } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md animate-scale-in rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Child Agent Login</h1>
          <p className="mt-1 text-sm text-gray-600">Access your agency dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="agent@childhub.com"
              required
            />
          </div>
          <div>
        ...
        <div className="mt-6 text-center text-sm">
          <span className="text-gray-600">New around here? </span>
          <Link to="/travelAgency/child/register" className="font-semibold text-primary-600 hover:underline">
            Register as Child Agent
          </Link>
        </div>
      </div>
    </div>
