...
export default function CreateBooking() {
  ...
  const urlWhitelabelId = searchParams.get('whitelabelId')
  ...
  useEffect(() => {
    if (offerType === 'whitelabel' && activeWhitelabels.length && !whitelabelId) {
      const targetId = urlWhitelabelId || (activeWhitelabels[0] ? String(activeWhitelabels[0]._id) : '')
      const found = activeWhitelabels.find(w => String(w._id) === targetId) || activeWhitelabels[0]
      if (found) {
        setWhitelabelId(String(found._id))
        setBasePackagePrice(found.finalPrice || 0)
        const cap = found.originalPackage?.maxCapacity ?? found.maxCapacity ?? null
        const rem = found.remainingCapacity ?? cap
        setMaxCapacity(cap)
        setRemainingCapacity(rem)
        setTravelDate(toDateOnly(found.originalPackage?.startDate))
      }
    }
    if (offerType === 'package' && availablePackages?.length && !packageId) {
      const targetId = urlPackageId || (availablePackages[0] ? String(availablePackages[0]._id) : '')
      const found = availablePackages.find(p => String(p._id) === targetId) || availablePackages[0]
      if (found) {
        ...
        setTravelDate(toDateOnly(found.startDate))
      }
    }
            ...
            <div className="flex items-center gap-2">
              ...
              {maxCapacity != null && (
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                  (remainingCapacity ?? maxCapacity) != null && travelers.length + 1 >= (remainingCapacity ?? maxCapacity)
