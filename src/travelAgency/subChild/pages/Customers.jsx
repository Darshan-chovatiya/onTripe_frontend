...
export default function SubChildCustomers() {
    ...
    try {
      ...
      setRows(prev => prev.map(r =>
        r.agencyCustomerId === editTarget.agencyCustomerId
          ? { ...r, name: editForm.name, email: editForm.email, notes: editForm.notes }
          : r
      ))
  ...
  const handleToggleActive = async (row) => {
    ...
    try {
      ...
      setRows(prev => prev.map(r =>
        r.agencyCustomerId === row.agencyCustomerId ? { ...r, isActive: !r.isActive } : r
      ))
                ...
                {rows.map(c => {
                  ...
                  <tr key={c.id} className="transition-colors hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-2.5 align-middle text-xs text-gray-500">
                      {c.lastActivity
                        ? new Date(c.lastActivity).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
                        : '—'}
                    </td>
