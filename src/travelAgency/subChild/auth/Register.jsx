...
export default function SubChildRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      toast.success('Your Sub-Agent account is ready! Redirecting to login...')
      setTimeout(() => navigate('/travelAgency/subchild/login'), 2000)
    } else {
      ...
      <div className="w-full max-w-md animate-scale-in rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="btn-primary w-full shadow-primary-500/10">
            {isLoading ? <div className="flex items-center justify-center gap-2"><Loader size="sm" color="white" /> Registering...</div> : 'Join as Sub Agent'}
          </button>
        </form>
