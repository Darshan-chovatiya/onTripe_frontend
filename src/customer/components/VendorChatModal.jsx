...
export default function VendorChatModal({ isOpen, onClose, bookingId, vendor }) {
  ...
  const fetchMessages = async () => {
    try {
      const res = await getVendorChatMessages(bookingId, vendor._id)
      if (res.data?.success) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const res = await sendVendorChatMessage(bookingId, vendor._id, formData)
      if (res.data?.success) {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              <p className="text-sm">Say hi to {vendor.name}!</p>
            </div>
          ) : (
